...
db = SQLAlchemy()
Column, String, Integer, DateTime = (db.Column, db.String,
                                    db.Integer, db.DateTime)
...
def user_table():
    return Table('user',Users.metadata)


def add_user(password, email, engine):
    table = user_table()    
...
def show_users(engine):
    table = user_table()
    statement = select([table.c.email])
...
#  def send_password_key(email,engine):
    #  '''
    #  ensure email exists
